def create_log_dict(file_list):
    ...
    for order in file_list:
        ...
        if order_list[0] not in log_dict:
            ...
            log_dict[order_list[0]]['visit_days'] = (
                log_dict[order_list[0]]['visit_days'].union(
                    set([order_list[2]])
                )
            )
        else:
...
def analyze_log(path_to_file):
    ...
    response_list.append(menu.difference(
        set(log_dict['joao']['dishes'].keys())
    ))
    response_list.append(week_days.difference(
        log_dict['joao']['visit_days']
    ))
